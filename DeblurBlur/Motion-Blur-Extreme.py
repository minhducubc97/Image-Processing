...
# Blur
blurred_array = np.multiply(image_fft, kernel_fft**5) # add power to kernel_fft to get more motion
...
# Save
blurred_image = Image.fromarray( 255*(np.abs(blurred_array)/np.max(blurred_array)).real).convert('RGB').save('[DOPE]motion-blurred-1.jpg')
deblurred_image = Image.fromarray( 255*(np.abs(deblurred_array)/np.max(deblurred_array)).real).convert('RGB').save('[DOPE]motion-deblurred-1.jpg')
